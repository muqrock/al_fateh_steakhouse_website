...
const CustomerNavbar: React.FC<CustomerNavbarProps> = ({ transparent = false, currentPage }) => {
    ...
    const navStyle = transparent ? 'bg-black/30 absolute top-0 left-0 right-0 z-50' : 'bg-white shadow-md border-b border-gray-200';

    const textStyle = transparent ? 'text-white drop-shadow-lg' : 'text-gray-800';

    const hoverStyle = transparent ? 'hover:text-orange-300' : 'hover:text-orange-600';
    ...
    return (
        <nav className={`flex items-center justify-between p-6 text-xl ${navStyle}`}>
            {/* Logo and Brand */}
            <a href="/" className="mr-8 flex items-center gap-3">
                <img
            ...
            <div className="flex items-center gap-6 text-lg">
                ...
                <a href="/reservation" className={getNavLinkClass('reservation')}>
                    Reservation
                </a>
                <a href="/review" className={getNavLinkClass('review')}>
                    Review
                </a>
                ...
                {isCustomerLoggedIn ? (
                    <div className="relative ml-6" ref={dropdownRef}>
                        <button
                            ...
                            className={`flex items-center gap-2 transition-colors duration-300 ${textStyle} ${hoverStyle} cursor-pointer border-none bg-transparent`}
                        >
                            {/* Profile Icon */}
                        ...
                        {isDropdownOpen && (
                            <div className="absolute right-0 z-50 mt-2 w-48 rounded-lg border border-gray-200 bg-white py-2 shadow-lg">
                                <button
                                    ...
                                    className="flex w-full items-center gap-2 px-4 py-2 text-left text-gray-700 transition-colors duration-200 hover:bg-gray-100"
                                >
                                ...
                                <button
                                    ...
                                    className="flex w-full items-center gap-2 px-4 py-2 text-left text-red-600 transition-colors duration-200 hover:bg-red-50"
                                >
                                    <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path
                                            strokeLinecap="round"
                    ...
                    <button
                        ...
                        className={`ml-6 transition-colors duration-300 ${textStyle} ${hoverStyle} cursor-pointer border-none bg-transparent text-lg font-medium`}
                    >
                        Login
                    </button>
                )}
