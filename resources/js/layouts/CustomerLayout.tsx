...
const CustomerLayout: React.FC<CustomerLayoutProps> = ({
    ...
    const contentStyle = fullHeight ? { flex: 1, display: 'flex', flexDirection: 'column' as const } : { flex: 1 };

    const overlayClass = fullHeight ? 'text-white flex flex-col min-h-screen' : '';
    ...
    return (
        <div style={containerStyle} className={overlayClass}>
            ...
            <main style={contentStyle} className={transparentNav ? 'relative z-10 flex-1' : 'z-10 flex-1'}>
                <div className={transparentNav ? 'flex flex-1 flex-col pt-24' : 'flex flex-1 flex-col'}>{children}</div>
            </main>
        </div>
    );
};
